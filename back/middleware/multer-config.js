...
const storage = multer.diskStorage({ //où enregistrer les fichiers entrants
    ...
    filename: (req, file, callback) => { 
        ...
        callback(null, name + Date.now() +'.'+ extension); //ajoute un timestamp comme nom de fichier
    }
});

module.exports = multer({storage: storage}).single('image'); //exporte l'élément multer et passe la constante storage, en indiquant que nous gérons uniquement des images